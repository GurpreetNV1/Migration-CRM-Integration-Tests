...
| `test_lodgement_eligibility_flow.py::test_set_stream_rejects_an_invalid_value` | Setting an application's stream to something other than "vocational"/"higher" is rejected with 422. |
| `test_lodgement_eligibility_flow.py::test_set_stream_then_status_reflects_it` | Setting a valid stream is immediately reflected in the eligibility status response. |
| `test_lodgement_eligibility_flow.py::test_create_course_rejects_an_unknown_course_type` | Filing a course with an unrecognized course_type is rejected with 422. |
| `test_lodgement_eligibility_flow.py::test_create_and_list_courses` | A created course is durably retrievable afterward via the list endpoint. |
| `test_lodgement_eligibility_flow.py::test_upload_lodgement_document_rejects_an_unknown_doc_type` | Uploading a document with an unrecognized doc_type is rejected with 422. |
| `test_lodgement_eligibility_flow.py::test_upload_lodgement_document_for_application_target` | An application-scoped document upload is recorded with a real Drive file reference. |
| `test_lodgement_eligibility_flow.py::test_upload_lodgement_document_for_course_target_requires_course_id` | A course-scoped upload with no course_id named is rejected with 422. |
| `test_lodgement_eligibility_flow.py::test_eligibility_check_end_to_end_reaches_eligible` | A vocational stream with a 92-CRICOS-week diploma reaches "eligible" with the correct total-weeks breakdown persisted. |
| `test_lodgement_eligibility_flow.py::test_document_validity_check_is_blocked_before_eligibility_check_runs` | The document-validity check reports "pending" rather than a real verdict until the qualification check has run. |
| `test_lodgement_eligibility_flow.py::test_run_eligibility_checks_cascades_through_all_three` | The combined "run all checks" endpoint advances the qualification check to eligible while correctly leaving document validity and lodgement date pending when no documents are on file. |
| `test_lodgement_eligibility_flow.py::test_get_eligibility_status_reflects_persisted_state` | A plain status read reflects the last-persisted stream without recomputing anything. |
