"""Wires app.state for the copied User Service integration tests in this folder.

Adapted from server/services/01_user_service/tests/conftest.py, which always forces an
...
# This service's own test files (test_staff_ops_flow.py, test_contacts_flow.py) seed prerequisite
# rows into Admin-Module-owned config tabs (Role_Hierarchy, System_Config) directly via
# app.state.gateway -- fine against the in-memory stand-in, rejected by the real Gateway's
# ownership enforcement. See _foreign_tab_gateway.py, one level up, for why and how this is
# handled in real mode only.
