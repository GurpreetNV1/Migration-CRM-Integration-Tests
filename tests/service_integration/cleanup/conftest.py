"""Wires app.state for the copied Cleanup Service integration tests in this folder.

Adapted from server/services/18_cleanup_service/tests/conftest.py, which always forces
in-memory backends. This one branches on TEST_GATEWAY_MODE (set by
run_local_integration_tests.py) so the exact same test bodies below can run against either
Gateway backend -- the tests themselves only ever call TestClient(app); they have no idea which
Gateway implementation is behind app.state.gateway.

backup_check_mode stays "memory" and start_cleanup_scheduler stays False in both modes -- only
the Gateway axis is toggled here, same reasoning as the original conftest for the rest.
"""
...
# This service's own test_cleanup_flow.py seeds prerequisite rows directly into Audit_Log
# (audit-service-owned) and Reminders_History (reminder-service-owned) via app.state.gateway --
# fine against the in-memory stand-in, rejected by the real Gateway's ownership enforcement. See
...
if _mode == "real":
    ...
    build_app_state(
        ...
        Settings(
            ...
            backup_check_mode="memory",
            start_cleanup_scheduler=False,
        ),
    )
    ...
    build_app_state(
        app,
        Settings(data_gateway_mode="memory", backup_check_mode="memory", start_cleanup_scheduler=False),
    )
