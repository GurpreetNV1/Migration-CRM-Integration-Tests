"""Wires app.state for the copied Backup & Restore Service integration tests in this folder.

Adapted from server/services/16_backup_restore_service/tests/conftest.py, which always forces
...
account_client_mode stays "memory" and start_backup_scheduler stays False in both modes -- only
...
if _mode == "real":
    ...
    build_app_state(
        app,
        Settings(
            data_gateway_mode="memory", account_client_mode="memory", start_backup_scheduler=False
        ),
    )
