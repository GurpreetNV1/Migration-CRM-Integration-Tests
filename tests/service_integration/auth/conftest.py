"""Wires app.state for the copied Auth Service integration tests in this folder.

Adapted from server/services/13_auth_service/tests/conftest.py, which always forces dev-only
defaults. This one branches on TEST_GATEWAY_MODE (set by run_local_integration_tests.py) so the
...
identity_provider/otp_notifier stay at their dev-only "fake"/"memory" defaults in both modes --
only the Gateway axis is toggled here, same reasoning as the original conftest for the rest.
"""
