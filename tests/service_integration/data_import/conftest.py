"""Wires app.state for the copied Data Import Service integration tests in this folder.

Adapted from server/services/11_data_import_service/tests/conftest.py, which always forces an
...
job_runner_mode stays "inline" in both modes -- each submitted job must run synchronously rather
than racing a real background thread, same reasoning as the original conftest.
"""
...
if _mode == "real":
    ...
    build_app_state(
        app,
        Settings(data_gateway_mode="http", data_gateway_url=_gateway_url, job_runner_mode="inline"),
    )
else:
    build_app_state(app, Settings(data_gateway_mode="memory", job_runner_mode="inline"))
